---
title: "Shiftingn tasks - Markdown 2020"
...
* Possible __DVs__ for the test battery are proportion correct in the CD phase of the Shifting boxes task, the number of platform switches (or the number of switching mistakes) in the Task-switching task, and the session-2 performance (proportion correct) in the Shifting tray task.  
...
```{r echo=FALSE, include=FALSE}
ED_max.distractorS1 <- Shifting.boxes.data %>%
  filter(Condition == "ED")%>%
  filter(SessionED == 1)%>%
  group_by(chosen_distractor, Subject) %>% 
  count(chosen_distractor) %>%
  ...
  ungroup()%>%
  group_by(Subject) %>% 
  summarise(S1_max.distractor=max(n))

ED_max.distractorS2 <- Shifting.boxes.data %>%
  filter(Condition == "ED")%>%
  filter(SessionED == 2)%>%
  group_by(chosen_distractor, Subject) %>% 
  count(chosen_distractor) %>%
  ...
  ungroup()%>%
  group_by(Subject) %>% 
  summarise(S2_max.distractor=max(n))

ED_max.distractor<-ED_max.distractorS1%>%
  full_join(ED_max.distractorS2)

m1=median(ED_max.distractor$S1_max.distractor)
m2=median(ED_max.distractor$S2_max.distractor)

tt1<-t.test(ED_max.distractor$S1_max.distractor, ED_max.distractor$S2_max.distractor, paired=TRUE, alternative = "two.sided")
...
* **ED: Preferred **: Preferred distractor in Session 1: Mean = `r round(m1, 2)`, Preferred distractor in Session 2: Median = `r round(m2, 2)`
...
p1 <- 
  ggplot(
  data=plot_shifting_shelf_sm , aes(x=c(rep(1, length(plot_shifting_shelf_sm$Subject))), y=shelf_switching_mistakes)) +   geom_boxplot()+
...
p2 <- 
  ggplot(
  data=plot_shifting_shelf_switches , aes(x=c(rep(1, length(plot_shifting_shelf_switches$Subject))), y=shelf_switches)) +   geom_boxplot()+
