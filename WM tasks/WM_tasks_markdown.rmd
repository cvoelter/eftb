...
p1<-  ggplot(
  data=Updating_plot, aes(x=rep(1, 53), y=Updating_P1.mistakes)) +
...
p3<-  ggplot(
  data=Boxes_plotting, aes(x=rep(1, 53), y=boxes_mistakes)) +
  geom_boxplot(outlier.colour = "white")+
  ylim(0,2)+
  xlim(0,2)+
...
Grid_plotting<-WM_task_individual %>%
  mutate(Grid_Distance = round(Grid_Distance,2)) %>%
  add_count(Grid_Distance)

#max(Grid_plotting$Grid_Distance) 

 
p4<-  ggplot(
  data=Grid_plotting, aes(x=rep(1, 53), y=Grid_Distance)) +
  geom_boxplot(outlier.colour = "white")+
  ylim(0,2)+
  xlim(0,2)+
