...
* **Candidate DVs**: Session 1 performance
...
```{r echo=FALSE}
b1<-binom.test(x=sum(Inh.grid_individual_first_trial$Inh.grid.first.trial),n=length(Inh.grid_individual_first_trial$Inh.grid.first.trial), p=6/13)

```
* First trial correct vs chance value (6/13) in the **first trial of Session 1**: Number of individuals: `r b1$parameter`, Number of successes: `r round(b1$statistic,2)`, Mean = `r round(b1$estimate, 2)`, 95% CI [`r round(b1$conf.int[1],2)`, `r round(b1$conf.int[2],2)`], *p*=`r round(b1$p.value,3)`  
