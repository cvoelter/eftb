---
title: "Inhibition tasks - Markdown"
...
* Evidence for a __prepotent response__: Below-chance performance in all three inhibition tasks at least in session 1. Significant improvement in performance between the first and the second session in the Inhibition Boxes and Inhibition Cylinders task but not the Inhibition Grid task.  
...
####Materials  
We use a sliding platform attached to the wall of the enclosure and an apparatus (flap-door box) with 16 compartments (4 x 4) with opaque and transparent flap doors. The apparatus will be fixed on top of the sliding platforms. All compartments of the apparatus can be baited via the back of the apparatus.
In the inhibition task, seven transparent and six opaque doors are arranged on the grid. All of these 13 compartments are baited outside the subject’s view at the beginning of a trial with a quarter piece of banana. The transparent doors (but not the opaque doors) are blocked (by means of a screw) and cannot be opened. Subjects can obtain rewards behind opaque flap-doors.
...
####Materials  
We use a sliding platform on top of a table outside the enclosure and two boxes made of Perspex (15.5 x 10 x 12 cm), one transparent and one opaque.  Besides we use an occluder for the baiting of the boxes. The boxes contain two compartments. In the lower compartment filling material (toilet paper) serves as potential hiding place for the food reward (1/16 apple piece). The filling material can be extracted though an opening in the front side of the boxes. The upper compartment is only visible in the transparent box. There will be a piece of food (1/2 apple) on the intermediate ceiling in the upper compartment lays inaccessible to the subject. The opaque box always contains a reward hidden in the filling material that can be obtained by removing the filling material. 
...
```
```{r include=FALSE}
write.csv(Inh_task_individual_corr, file = "../EF_tasks/Inh_task_individual_corr.csv", row.names=FALSE)
```
