...
Shifting.data<-Shifting.data  %>%
  select(-proportion_switching_mistakes )
...
EF_task_data2<-EF_task_data %>%
  select(Subject, ED, CD,  proportion_switches, Shifting.tray.correct.S2,Inh.grid.correct1, Inh.cylinders.correct1, Inh.boxes.correct1, Updating_P1P2.correct, boxes_correct_newrange, Grid_Distance_newrange)
...
ggcorrplot(cormat, hc.order = TRUE,
    type = "lower", p.mat =p.mat, lab=FALSE, sig.level = 0.15)
...
model.MF2012 <- 'F1_WM =~  Updating_P1P2.correct   + boxes_correct_newrange + Grid_Distance_newrange
                ...
                F3_CommonEF =~  Inh.grid.correct1 + Inh.cylinders.correct1 + Inh.boxes.correct1 + proportion_switches + CD + Shifting.tray.correct.S2 + Updating_P1P2.correct+ boxes_correct_newrange + Grid_Distance_newrange+SR'
fit.MF2012 <- cfa(model.MF2012, data = EF_task_data,  orthogonal=FALSE, estimator="ML", missing="fiml", optim.method="nlminb")#, std.lv=TRUE)
...
# 
# parameterEstimates(fit.MF2012, standardized=TRUE) %>% 
#   filter(op == "=~") %>% 
...
model.3factors <- 'F1_WM =~  Updating_P1P2.correct + boxes_correct_newrange + Grid_Distance_newrange
                F2_Shifting =~  CD + proportion_switches + Shifting.tray.correct.S2
                  F3_Inhibition =~  Inh.grid.correct1 + Inh.cylinders.correct1 + Inh.boxes.correct1+SR'
...
fit.1factor <- cfa(model.1factor, data = EF_task_data,  orthogonal=FALSE, estimator="ML", missing="fiml", optim.method="nlminb")#, std.lv=TRUE)
...
model.2factors2 <- 'F1_3_WM_Inh =~  Updating_P1P2.correct + boxes_correct_newrange + Grid_Distance_newrange + Inh.grid.correct1 + Inh.cylinders.correct1 + Inh.boxes.correct1
                    F2_Shifting =~  CD + proportion_switches + Shifting.tray.correct.S2'
fit.2factors2  <- cfa(model.2factors2 , data = EF_task_data, missing="fiml",orthogonal=FALSE, estimator="ML", missing="fiml", optim.method="nlminb")#, std.lv=TRUE)
summary(fit.2factors2, fit.measures = TRUE)

anova(fit.2factors2 , fit.1factor)
anova(fit.2factors1 , fit.2factors2)
# Shifting=Inhibition
...
anova(fit.1factor, fit.3factors_ind)

anova(fit.3factors_ind, fit.MF2012)
anova(fit.3factors, fit.MF2012)
```

```{r fig.height=4}
lavaanPlot(model = fit.3factors_ind, node_options = list(shape = "box", fontname = "Helvetica"), edge_options = list(color = "grey"), coefs = TRUE, stand = FALSE,  sig = 1.00,  stars=TRUE)
```

```{r fig.height=4}
lavaanPlot(model = fit.2factors1, node_options = list(shape = "box", fontname = "Helvetica"), edge_options = list(color = "grey"), coefs = TRUE, stand = FALSE,  sig = 1.00,  stars=TRUE)
```

```{r}
lavaanPlot(model = fit.1factor, node_options = list(shape = "box", fontname = "Helvetica"), edge_options = list(color = "grey"), coefs = TRUE, stand = TRUE,  sig = 1.00)
```

```{r}
# 3 correlated factors
model.3factors <- 'F1_WM =~  Updating_WM + Box_WM + Grid_WM
                  F2_Shifting =~  Box_Shifting + Task_Switching + Tray_Shifting
                  F3_Inhibition =~  Grid_Inh + Cylinders_Inh + Box_Inh'
fit.3factors <- cfa(model.3factors, data = xdata, missing="fiml", std.lv=TRUE)

# WM=Shifting
model.2factors1 <- 'F1_2_WM_Shifting =~  Updating_WM + Box_WM + Grid_WM + Box_Shifting + Task_Switching + Tray_Shifting
                   F3_Inhibition =~  Grid_Inh + Cylinders_Inh + Box_Inh'
fit.2factors1  <- cfa(model.2factors1 , data = xdata, missing="fiml", std.lv=TRUE)

# WM=Inhibition
model.2factors2 <- 'F1_3_WM_Inh =~  Updating_WM + Box_WM + Grid_WM + Grid_Inh + Cylinders_Inh + Box_Inh
                    F2_Shifting =~  Box_Shifting + Task_Switching + Tray_Shifting'
fit.2factors2  <- cfa(model.2factors2 , data = xdata, missing="fiml", std.lv=TRUE)

# Shifting=Inhibition
model.2factors3 <- 'F1_WM =~  Updating_WM + Box_WM + Grid_WM
                    F2_3_Shifting_Inh=~ Box_Shifting + Task_Switching + Tray_Shifting + Grid_Inh + Cylinders_Inh + Box_Inh'
fit.2factors3  <- cfa(model.2factors3 , data = xdata, missing="fiml", std.lv=TRUE)

# 1 factor
model.1factor <- 'F1_CommonEF =~  Grid_Inh + Cylinders_Inh + Box_Inh + Box_Shifting + Task_Switching + Tray_Shifting + Updating_WM + Box_WM + Grid_WM'
fit.1factor <- cfa(model.1factor, data = xdata, missing="fiml", std.lv=TRUE)

# 3 independent factors
model.3factors_ind <- 'F1_WM =~  Updating_WM + Box_WM + Grid_WM
                  F2_Shifting =~  Box_Shifting + Task_Switching + Tray_Shifting
                  F3_Inhibition =~  Grid_Inh + Cylinders_Inh + Box_Inh'
fit.3factors_ind <- cfa(model.3factors_ind, data = xdata, orthogonal=TRUE, missing="fiml", std.lv=TRUE)
